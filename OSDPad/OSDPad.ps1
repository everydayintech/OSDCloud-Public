...
if ($Global:OSDPad) {
    $Global:OSDPad | ForEach-Object {
        $ScriptSelectionControl.Items.Add(($_.Path -replace '\.ps1$', '')) | Out-Null
...
function Set-OSDPadContent {
    if ($ScriptSelectionControl.SelectedValue -eq 'New PowerShell Script.ps1') {
        ...
        $Global:WorkingScript = $Global:OSDPad | Where-Object {($_.Path -replace '\.ps1$', '') -eq $ScriptSelectionControl.SelectedValue} | Select-Object -First 1
    ...
    foreach ($Item in $Hide) {
        ...
        if ($Item -eq 'Script' -and ($Global:WorkingScript.Name -notlike "*.md")) {
            $Global:XamlWindow.Height="140"
            $ScriptTextControl.Visibility = "Collapsed"
        }
        else {
            $Global:XamlWindow.Height="480"
            $ScriptTextControl.Visibility = "Visible"
        }
    }
    Write-Host -ForegroundColor DarkGray "========================================================================="
}
